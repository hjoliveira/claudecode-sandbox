#!/usr/bin/env bash
#
# sandbox-docker.sh — Run Claude Code in a Docker-based sandbox.
#
# Same interface as sandbox.sh but works on any OS with Docker installed.
# Does NOT require root on the host.
#
# Usage:
#   ./sandbox-docker.sh --dir /path/to/project --domains "api.anthropic.com,github.com" [-- claude args...]
#
set -euo pipefail
...
usage() {
    cat <<'USAGE'
Usage: ./sandbox-docker.sh [OPTIONS] [-- claude-code-args...]
...
Examples:
  ./sandbox-docker.sh --dir ./my-project --domains "api.anthropic.com"
  ./sandbox-docker.sh --dir /home/user/code --domains "api.anthropic.com,github.com" -- --model sonnet
USAGE
    exit 0
}
